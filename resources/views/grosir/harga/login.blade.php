...
<div class="login-form-area">
    ...
    <div class="auth-panel {{ $showLogin ? 'on' : '' }}" id="panel-login">
        <form method="POST" action="{{ route('login') }}">
            ...
            {{-- ===== KODE AKTIVASI (DIPERBAIKI: Mengambil nomor admin dari database) ===== --}}
            @php
                // Tampilkan terbuka otomatis jika ada error, old input, atau URL ?expired=1
                $showCodeField = $errors->has('activation_code') || old('activation_code') !== null || request()->query('expired') === '1';
                
                // Ambil nomor WA admin dari tabel settings (sesuaikan key 'admin_wa' dengan database Anda)
                $adminWa = \App\Models\Setting::get('admin_wa', '');
                $adminWaClean = preg_replace('/[^0-9]/', '', (string) $adminWa);
                
                // Pastikan format nomor diawali '62' (Standar WhatsApp API)
                if (!empty($adminWaClean) && strpos($adminWaClean, '62') !== 0) {
                    $adminWaClean = '62' . ltrim($adminWaClean, '0');
                }
                ...
                $waUrl = !empty($adminWaClean) ? "https://wa.me/{$adminWaClean}?text={$waMessage}" : '';
            @endphp
            ...
            <div class="act-code-wrap {{ $showCodeField ? 'open' : '' }}" id="actCodeWrap">
                ...
                <div class="act-code-body">
                    ...
                    @if(!empty($waUrl))
                        <a href="{{ $waUrl }}" target="_blank" rel="noopener" class="b b-wa" style="margin-top:8px; display:inline-flex; align-items:center; gap:6px;">
                            <i class="fab fa-whatsapp"></i> Minta Kode Aktivasi via WhatsApp
                        </a>
                    @else
                        <div style="margin-top:8px; font-size:0.75rem; color:#ef4444;">
                            <i class="fas fa-exclamation-triangle"></i> Nomor WhatsApp admin belum diatur di pengaturan.
                        </div>
                    @endif
                </div>
            </div>

            <button type="submit" class="b bp b-full" style="margin-top: 16px;"><i class="fas fa-sign-in-alt"></i> Masuk</button>
        </form>
